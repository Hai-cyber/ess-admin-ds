...
const INTEGRATION_KEY_DESCRIPTIONS = {
  ...
  ODOO_BASE_URL: 'Odoo base URL (tenant override, e.g. https://hais-lab.odoo.com)',
  ODOO_DB_NAME: 'Odoo database name for this tenant',
  ODOO_LOGIN: 'Odoo login/user for this tenant',
  ODOO_API_TOKEN: 'Odoo API token for this tenant',
  ODOO_PASSWORD: 'Optional Odoo password for this tenant (prefer API token)',
  ODOO_TAG_FOUNDER: 'Label for Founder tag',
  ODOO_TAG_FOUNDER_TRIAL: 'Label for Founder Trial tag',
  ODOO_TAG_KC_CLUB: 'Label for KC Club tag',
  ODOO_TAG_KOLLEGENSCLUB: 'Label for Kollegensclub tag',
  ODOO_FOUNDER_CREATE_WEBHOOK: 'Legacy Make webhook fallback for Founder/KC register sync',
  ODOO_FOUNDER_VERIFY_WEBHOOK: 'Legacy Make webhook fallback for Founder/KC verify sync',
...
async function getFounderRuntimeConfig(env, companyId) {
  ...
  return {
    ...
    odooBaseUrl: settingsMap.ODOO_BASE_URL || String(env.ODOO_BASE_URL || '').trim(),
    odooDbName: settingsMap.ODOO_DB_NAME || String(env.ODOO_DB_NAME || '').trim(),
    odooLogin: settingsMap.ODOO_LOGIN || String(env.ODOO_LOGIN || '').trim(),
    odooApiToken: settingsMap.ODOO_API_TOKEN || String(env.ODOO_API_TOKEN || '').trim(),
    odooPassword: settingsMap.ODOO_PASSWORD || String(env.ODOO_PASSWORD || '').trim(),
    odooTagFounder: settingsMap.ODOO_TAG_FOUNDER || String(env.ODOO_TAG_FOUNDER || '').trim(),
    odooTagFounderTrial: settingsMap.ODOO_TAG_FOUNDER_TRIAL || String(env.ODOO_TAG_FOUNDER_TRIAL || '').trim(),
    odooTagKcClub: settingsMap.ODOO_TAG_KC_CLUB || String(env.ODOO_TAG_KC_CLUB || '').trim(),
    odooTagKollegensclub: settingsMap.ODOO_TAG_KOLLEGENSCLUB || String(env.ODOO_TAG_KOLLEGENSCLUB || '').trim(),
...
function normalizeFounderLevelForOdoo(rawLevel) {
  const level = String(rawLevel || '').trim();
  if (!level) return 'Trial';
  if (level.toLowerCase() === 'trial') return 'Trial';
  return level.charAt(0).toUpperCase() + level.slice(1);
}

function normalizeOdooRegisterSyncState(rawState) {
  return String(rawState || '').trim().toLowerCase();
}

function hasSuccessfulOdooRegisterSync(customerRecord) {
  return normalizeOdooRegisterSyncState(customerRecord?.odoo_register_sync_state) === 'synced';
}

async function updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
  state,
  error = null,
  syncedAt = null,
  ...
  await env.DB.prepare(`
    UPDATE customers
    SET odoo_register_sync_state = ?,
        odoo_register_sync_error = ?,
        odoo_register_synced_at = ?,
        odoo_register_sync_attempts = COALESCE(odoo_register_sync_attempts, 0) + ?,
        updated_at = ?,
        updated_by = ?
...
function getFounderRegisterPayload({
  ...
  companyId,
  nowIso,
  optInTimestampIso = nowIso,
  membershipType,
  optInText = FOUNDER_OPT_IN_TEXT,
  founderTermsAccepted = true,
  kcTermsAccepted = false,
  founderLevel = 'Trial',
  lastReminderDateIso = nowIso,
  totalSpent = 0,
  numberOfVisits = 0,
  notes = ''
}) {
  const berlinDateTime = formatIsoToBerlinDateTime(optInTimestampIso || nowIso);
  const berlinDate = formatIsoToBerlinDate(lastReminderDateIso || nowIso);
  const normalizedVisits = Number.isFinite(Number(numberOfVisits)) ? Number(numberOfVisits) : 0;
  const normalizedTotalSpent = Number.isFinite(Number(totalSpent)) ? Number(totalSpent) : 0;
  ...
  return {
    ...
    x_studio_founder_level: normalizeFounderLevelForOdoo(founderLevel),
    x_studio_last_reminder_date: berlinDate,
    x_studio_total_spent: normalizedTotalSpent,
    x_number_of_visits: normalizedVisits,
    x_studio_notes: String(notes || '').trim() || (kcTermsAccepted ? 'KC Form Registration' : 'Founder Form Registration'),
    company_id: companyId
  };
}

function buildFounderRegisterPayloadFromCustomerRecord(customerRecord, companyId, fallbackNowIso) {
  const nowIso = String(fallbackNowIso || new Date().toISOString());
  const kcTermsAccepted = Number(customerRecord?.kc_terms_accepted) === 1;
  const founderTermsAccepted = Number(customerRecord?.founder_terms_accepted) === 1;

  return getFounderRegisterPayload({
    name: String(customerRecord?.name || '').trim(),
    phone: String(customerRecord?.phone || '').trim(),
    email: String(customerRecord?.email || '').trim(),
    companyId,
    nowIso,
    optInTimestampIso: customerRecord?.opt_in_timestamp || nowIso,
    membershipType: kcTermsAccepted ? 'KC' : FOUNDER_DEFAULT_MEMBERSHIP_TYPE,
    optInText: String(customerRecord?.opt_in_text || FOUNDER_OPT_IN_TEXT).trim() || FOUNDER_OPT_IN_TEXT,
    founderTermsAccepted,
    kcTermsAccepted,
    founderLevel: customerRecord?.founder_level || 'Trial',
    lastReminderDateIso: customerRecord?.last_reminder_date || nowIso,
    totalSpent: customerRecord?.total_spent ?? 0,
    numberOfVisits: customerRecord?.number_of_visits ?? 0,
    notes: String(customerRecord?.notes || '').trim()
  });
}

function normalizeOdooBaseUrl(rawBaseUrl) {
...
function normalizeLegacyWebhookUrl(rawWebhookUrl) {
  const trimmed = String(rawWebhookUrl || '').trim();
  if (!trimmed) return '';
...
function getOdooApiConfig(env, runtimeConfig = null) {
  const baseUrl = normalizeOdooBaseUrl(runtimeConfig?.odooBaseUrl || env.ODOO_BASE_URL || '');
  const dbName = String(runtimeConfig?.odooDbName || env.ODOO_DB_NAME || '').trim();
  const login = String(runtimeConfig?.odooLogin || env.ODOO_LOGIN || '').trim();
  const secret = String(runtimeConfig?.odooApiToken || runtimeConfig?.odooPassword || env.ODOO_API_TOKEN || env.ODOO_PASSWORD || '').trim();
  ...
  return {
    ...
    login,
    secret,
    isConfigured: !!(baseUrl && dbName && login && secret)
  };
}

function truncateOdooError(rawText, maxLength = 1000) {
  const text = String(rawText || '').trim();
  if (!text) return '';
  return text.length > maxLength ? `${text.slice(0, maxLength - 3)}...` : text;
}

function extractOdooRpcErrorMessage(errorPayload) {
  ...
  const detail = String(errorPayload?.data?.debug || '').split('\n')[0].trim();
  if (!detail || detail === primary) return truncateOdooError(primary);
  return truncateOdooError(`${primary} (${detail})`);
}

async function callOdooJsonRpc(config, payload) {
  const endpoint = `${config.baseUrl}/jsonrpc`;
  let response;
  ...
  try {
    ...
    return { ok: false, error: `Odoo API network error: ${truncateOdooError(error?.message || String(error))}` };
  }

  if (!response.ok) {
    const text = await response.text().catch(() => '');
    return { ok: false, error: `Odoo API HTTP ${response.status}: ${truncateOdooError(text || response.statusText)}` };
  }

  const body = await response.json().catch(() => null);
  if (!body || typeof body !== 'object') {
    return { ok: false, error: 'Odoo API returned an invalid JSON-RPC payload.' };
  }

  if (body.error) {
    return { ok: false, error: extractOdooRpcErrorMessage(body.error) };
  }
...
async function authenticateOdoo(config) {
  const rpcPayload = {
    ...
    params: {
      service: 'common',
      method: 'authenticate',
      args: [config.dbName, config.login, config.secret, {}]
    },
    id: Date.now()
  };

  const rpcResult = await callOdooJsonRpc(config, rpcPayload);
  if (!rpcResult.ok) return rpcResult;

  const uid = Number(rpcResult.result || 0);
  if (!Number.isInteger(uid) || uid <= 0) {
    return { ok: false, error: 'Odoo authentication failed. Check DB name, login, and API token/password.' };
  }

  return { ok: true, uid };
}

async function odooExecuteKw(config, uid, secret, model, method, methodArgs = [], methodKw = {}) {
  const rpcPayload = {
    jsonrpc: '2.0',
    method: 'call',
    params: {
      service: 'object',
      method: 'execute_kw',
      args: [config.dbName, uid, secret, model, method, methodArgs, methodKw]
    },
    id: Date.now()
  };

  return callOdooJsonRpc(config, rpcPayload);
}

async function getOdooModelFieldMap(config, uid, secret, model) {
  const fieldsResult = await odooExecuteKw(
    config,
    uid,
    secret,
    model,
  ...
  if (!fieldsResult.ok) {
    return {
      ok: true,
      fieldMap: null,
      warning: `Unable to introspect Odoo fields for ${model}: ${fieldsResult.error}`
    };
  }

  const fieldMap = fieldsResult.result && typeof fieldsResult.result === 'object'
    ? fieldsResult.result
    : null;
...
function filterOdooValuesByFieldMap(rawValues, fieldMap = null) {
  const filtered = {};
  for (const [key, value] of Object.entries(rawValues || {})) {
    if (!key || key.startsWith('_')) continue;
    if (key === 'id' || key === 'company_id') continue;
    if (value === undefined) continue;
    if (fieldMap && typeof fieldMap === 'object' && !(key in fieldMap)) continue;
    filtered[key] = value;
  }
  return filtered;
}

async function findOdooPartnerByPhone(config, uid, secret, phone, fieldMap = null) {
  const hasPhoneField = !fieldMap || typeof fieldMap !== 'object' || !!fieldMap.phone;
  const hasMobileField = !!(fieldMap && typeof fieldMap === 'object' && fieldMap.mobile);

  let domain;
  if (hasPhoneField && hasMobileField) {
    domain = ['|', ['phone', '=', phone], ['mobile', '=', phone]];
  } else if (hasPhoneField) {
    domain = [['phone', '=', phone]];
  } else if (hasMobileField) {
    domain = [['mobile', '=', phone]];
  } else {
    return { ok: true, partner: null };
  }

  const fields = ['id'];
  if (hasPhoneField) fields.push('phone');
  if (hasMobileField) fields.push('mobile');
  if (!fieldMap || fieldMap.category_id) fields.push('category_id');

  const lookupResult = await odooExecuteKw(
    config,
    uid,
    secret,
    'res.partner',
  ...
  const rows = Array.isArray(lookupResult.result) ? lookupResult.result : [];
  return { ok: true, partner: rows[0] || null };
}
...
function getFounderOdooTagLabels(payload, runtimeConfig = null) {
  const membershipType = String(payload?.x_studio_membership_type || '').trim().toLowerCase();
  const founderLevel = String(payload?.x_studio_founder_level || '').trim().toLowerCase();
  const founderTag = String(runtimeConfig?.odooTagFounder || 'Founder').trim();
  const founderTrialTag = String(runtimeConfig?.odooTagFounderTrial || 'Founder Trial').trim();
  const kcClubTag = String(runtimeConfig?.odooTagKcClub || 'KC Club').trim();
  const kollegensclubTag = String(runtimeConfig?.odooTagKollegensclub || 'Kollegensclub').trim();

  if (membershipType === 'kc') {
    return uniqueNonEmptyStrings([kcClubTag, kollegensclubTag]);
  }

  if (!founderLevel || founderLevel === 'trial') {
    return uniqueNonEmptyStrings([founderTag, founderTrialTag]);
  }

  return uniqueNonEmptyStrings([founderTag]);
}

function getAllManagedFounderOdooTagLabels(runtimeConfig = null) {
  return uniqueNonEmptyStrings([
    String(runtimeConfig?.odooTagFounder || 'Founder').trim(),
    String(runtimeConfig?.odooTagFounderTrial || 'Founder Trial').trim(),
    String(runtimeConfig?.odooTagKcClub || 'KC Club').trim(),
    String(runtimeConfig?.odooTagKollegensclub || 'Kollegensclub').trim()
  ]);
}

async function findOdooPartnerCategoryIdByLabel(config, uid, secret, label) {
  const searchResult = await odooExecuteKw(
    config,
    uid,
    secret,
  ...
  const existingRows = Array.isArray(searchResult.result) ? searchResult.result : [];
  const existingId = Number(existingRows[0]?.id || 0);
  return { ok: true, categoryId: existingId > 0 ? existingId : null };
}

async function getOrCreateOdooPartnerCategoryId(config, uid, secret, label) {
  const searchResult = await findOdooPartnerCategoryIdByLabel(config, uid, secret, label);

  if (!searchResult.ok) return searchResult;

  const existingId = Number(searchResult.categoryId || 0);
  if (existingId > 0) {
    return { ok: true, categoryId: existingId };
  }

  const createResult = await odooExecuteKw(
    config,
    uid,
    secret,
    'res.partner.category',
  ...
  const createdId = Number(createResult.result || 0);
  if (createdId <= 0) {
    return { ok: false, error: `Failed to create Odoo tag: ${label}` };
  }

  return { ok: true, categoryId: createdId };
}

function normalizeOdooCategoryIdList(rawValue) {
...
function extractOdooIncompatibleFieldName(rawError) {
  const error = String(rawError || '');

  const patterns = [
    /Invalid field\s+(?:[a-zA-Z0-9_]+\.)+([a-zA-Z0-9_]+)/i,
    /Invalid field\s+['"]([a-zA-Z0-9_]+)['"]/i,
    /Wrong value for\s+(?:[a-zA-Z0-9_]+\.)+([a-zA-Z0-9_]+)\s*:/i
  ];
...
async function executeOdooMutationWithFallback({
  config,
  uid,
  secret,
  model,
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const valueKeys = Object.keys(mutableValues);
    if (!valueKeys.length) {
      if (method === 'write') {
        return {
          ok: true,
          skipped: true,
          warning: removedFields.length
            ? `Odoo write skipped after pruning incompatible fields: ${removedFields.join(', ')}`
            : 'Odoo write skipped because there are no compatible fields.'
        };
      }

      return {
        ok: false,
        error: removedFields.length
          ? `No compatible Odoo create fields left after pruning: ${removedFields.join(', ')}`
          : 'No compatible Odoo create fields available.'
      };
    }

    const methodArgs = method === 'create'
      ? [mutableValues]
      : [[recordId], mutableValues];

    const result = await odooExecuteKw(config, uid, secret, model, method, methodArgs, {});
    if (result.ok) {
      return {
        ...result,
        warning: removedFields.length
          ? `Odoo ${method} pruned incompatible fields: ${removedFields.join(', ')}`
          : ''
      };
    }

    const incompatibleField = extractOdooIncompatibleFieldName(result.error);
    if (!incompatibleField || !(incompatibleField in mutableValues)) {
      return result;
    }

    delete mutableValues[incompatibleField];
    removedFields.push(incompatibleField);
  }

  return { ok: false, error: `Odoo ${method} failed after compatibility retries.` };
}

async function assignFounderTagsToOdooPartner(config, uid, secret, partnerId, payload, runtimeConfig = null, partnerFieldMap = null) {
  if (partnerFieldMap && !(partnerFieldMap.category_id)) {
    return { ok: true, skipped: true };
  }

  const desiredLabels = getFounderOdooTagLabels(payload, runtimeConfig);
  if (!desiredLabels.length) {
    return { ok: true, skipped: true };
  }

  const managedLabels = getAllManagedFounderOdooTagLabels(runtimeConfig);
  const labelsToRemove = managedLabels.filter((label) => !desiredLabels.includes(label));

  const resolvedTagIds = [];
  for (const label of desiredLabels) {
    const result = await getOrCreateOdooPartnerCategoryId(config, uid, secret, label);
    if (!result.ok) {
      return {
        ok: false,
        nonCritical: true,
        error: `Tag resolution failed for "${label}": ${result.error}`
      };
    }

    resolvedTagIds.push(result.categoryId);
  }

  const resolvedRemovalIds = [];
  for (const label of labelsToRemove) {
    const result = await findOdooPartnerCategoryIdByLabel(config, uid, secret, label);
    if (!result.ok) {
      return {
        ok: false,
        nonCritical: true,
        error: `Tag lookup failed for "${label}": ${result.error}`
      };
    }

    if (Number(result.categoryId || 0) > 0) {
      resolvedRemovalIds.push(Number(result.categoryId));
    }
  }

  const currentPartnerResult = await odooExecuteKw(
    config,
    uid,
    secret,
    'res.partner',
    'read',
    [[partnerId], ['category_id']],
    {}
  );

  if (!currentPartnerResult.ok) {
    return {
      ok: false,
      nonCritical: true,
      error: `Unable to read current partner tags: ${currentPartnerResult.error}`
    };
  }

  const rows = Array.isArray(currentPartnerResult.result) ? currentPartnerResult.result : [];
  const existingIds = normalizeOdooCategoryIdList(rows[0]?.category_id);
  const filteredExistingIds = existingIds.filter((id) => !resolvedRemovalIds.includes(id));
  const mergedIds = Array.from(new Set([...filteredExistingIds, ...resolvedTagIds]));

  const writeResult = await odooExecuteKw(
    config,
    uid,
    secret,
    'res.partner',
    'write',
    [[partnerId], { category_id: [[6, 0, mergedIds]] }],
    {}
  );
  ...
  if (!writeResult.ok) {
    return {
      ok: false,
      nonCritical: true,
      error: `Unable to write partner tags: ${writeResult.error}`
    };
  }
...
async function syncFounderProfileToOdooApi(env, payload, runtimeConfig = null, mode = 'register') {
  const config = getOdooApiConfig(env, runtimeConfig);
  if (!config.isConfigured) {
    return { ok: true, skipped: true };
  }

  const authResult = await authenticateOdoo(config);
  if (!authResult.ok) {
    return authResult;
  }

  const uid = authResult.uid;
  const model = String(payload?._model || 'res.partner').trim() || 'res.partner';
  const fieldMapResult = await getOdooModelFieldMap(config, uid, config.secret, model);
  if (!fieldMapResult.ok) {
    return fieldMapResult;
  }

  if (fieldMapResult.warning) {
    console.warn(fieldMapResult.warning);
  }

  const normalizedPhone = normalizeFounderPhone(payload?.phone || '');
  if (!normalizedPhone) {
    return { ok: false, error: 'Odoo sync requires a valid phone value.' };
  }

  const partnerLookupResult = await findOdooPartnerByPhone(
    config,
    uid,
    config.secret,
    normalizedPhone,
  ...
  const values = filterOdooValuesByFieldMap(rawValues, fieldMapResult.fieldMap);

  if (!values.name && mode === 'register') {
    values.name = String(payload?.name || normalizedPhone).trim() || normalizedPhone;
  }

  let partnerId = Number(partnerLookupResult.partner?.id || 0);
  let created = false;

  if (partnerId > 0) {
    if (Object.keys(values).length) {
      const writeResult = await executeOdooMutationWithFallback({
        config,
        uid,
    ...
    const createResult = await executeOdooMutationWithFallback({
      config,
      uid,
      secret: config.secret,
      model,
    ...
    if (createResult.warning) {
      console.warn(createResult.warning);
    }

    partnerId = Number(createResult.result || 0);
    if (partnerId <= 0) {
      return { ok: false, error: 'Odoo partner create returned an invalid ID.' };
    }

    created = true;
  }

  const tagResult = await assignFounderTagsToOdooPartner(
    config,
    uid,
    config.secret,
    partnerId,
  ...
  if (!tagResult.ok) {
    console.warn('Odoo partner tag assignment warning:', tagResult.error);
    return {
      ok: true,
      partnerId,
      created,
  ...
  return { ok: true, partnerId, created };
}

async function syncFounderProfileToLegacyWebhook(env, payload, runtimeConfig = null, mode = 'register') {
  const webhookRaw = mode === 'verify'
    ? runtimeConfig?.odooFounderVerifyWebhook || env.ODOO_FOUNDER_VERIFY_WEBHOOK || ''
    : runtimeConfig?.odooFounderCreateWebhook || env.ODOO_FOUNDER_CREATE_WEBHOOK || '';
  const webhookUrl = normalizeLegacyWebhookUrl(webhookRaw);
  ...
  body.set('_sync_mode', mode);
  body.set('_sync_source', 'worker_legacy_fallback');
  ...
  try {
    ...
    return {
      ok: false,
      error: `Legacy Odoo webhook network error: ${truncateOdooError(error?.message || String(error))}`
    };
  }
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      error: `Legacy Odoo webhook HTTP ${response.status}: ${truncateOdooError(text || response.statusText)}`
    };
  }
  ...
  if (contentType.includes('application/json')) {
    ...
    if (status === 'error' || payload?.ok === false || payload?.success === false) {
      return {
        ok: false,
        error: truncateOdooError(payload?.message || payload?.error || 'Legacy Odoo webhook returned an error')
      };
    }
  }
...
async function syncFounderRegisterToOdoo(env, payload, runtimeConfig = null) {
  const primary = await syncFounderProfileToOdooApi(env, payload, runtimeConfig, 'register');
  if (!primary.ok || !primary.skipped) {
    return primary;
  }

  const fallback = await syncFounderProfileToLegacyWebhook(env, payload, runtimeConfig, 'register');
  if (!fallback.ok) {
    return fallback;
  }

  return fallback.skipped ? primary : fallback;
}

async function syncFounderVerifyToOdoo(env, payload, runtimeConfig = null) {
  const primary = await syncFounderProfileToOdooApi(env, payload, runtimeConfig, 'verify');
  if (!primary.ok || !primary.skipped) {
    return primary;
  }

  const fallback = await syncFounderProfileToLegacyWebhook(env, payload, runtimeConfig, 'verify');
  if (!fallback.ok) {
    return fallback;
  }
...
async function handleFounderOtpVerificationRequest(request, env, companyId, options = {}) {
  ...
  try {
    ...
    const customerRecord = await env.DB.prepare(
      `SELECT name, phone, email, opt_in_text, opt_in_timestamp, founder_terms_accepted, kc_terms_accepted, founder_level, last_reminder_date, total_spent, number_of_visits, notes, odoo_register_sync_state FROM customers WHERE company_id = ? AND phone = ? LIMIT 1`
    ).bind(companyId, phone).first();
    ...
    if (!hasSuccessfulOdooRegisterSync(customerRecord)) {
      const registerSyncPayload = buildFounderRegisterPayloadFromCustomerRecord(customerRecord, companyId, now);
      const registerSyncResult = await syncFounderRegisterToOdoo(env, registerSyncPayload, founderRuntimeConfig);

      if (registerSyncResult.skipped) {
        await updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
          state: 'not_configured',
          error: null,
          syncedAt: null,
          incrementAttempts: false
        });
      } else if (!registerSyncResult.ok) {
        console.warn('Founder verify pre-sync create failed:', registerSyncResult.error);
        await updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
          state: 'failed',
          error: registerSyncResult.error || 'Register sync failed during verify',
          syncedAt: null,
          incrementAttempts: true
        });
        return respond('error', 'Profil konnte derzeit nicht mit Odoo synchronisiert werden. Bitte erneut versuchen.', 502);
      } else {
        await updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
          state: 'synced',
          error: null,
    ...
    const verifyFounderTermsAccepted = Number(customerRecord?.founder_terms_accepted) === 1 ? 'yes' : 'no';
    const verifyKcTermsAccepted = Number(customerRecord?.kc_terms_accepted) === 1 ? 'yes' : 'no';

    const verifySyncResult = await syncFounderVerifyToOdoo(env, {
      _model: 'res.partner',
      phone,
      x_studio_membership_type: verifyMembershipType,
      x_studio_founder_terms_accepted: verifyFounderTermsAccepted,
      x_studio_kc_terms_accepted: verifyKcTermsAccepted,
    ...
    if (!verifySyncResult.ok && !verifySyncResult.skipped) {
      console.warn('Founder verify Odoo sync failed:', verifySyncResult.error);
      return respond('error', 'Verifizierung konnte derzeit nicht mit Odoo synchronisiert werden. Bitte erneut versuchen.', 502);
    }
    ...
    if ((url.pathname === "/api/founder/register" || url.pathname === '/api/kc/register') && request.method === "POST") {
      try {
        ...
        const existing = await env.DB.prepare(
          `SELECT id, founder_status, founder_terms_accepted, kc_terms_accepted, odoo_register_sync_state FROM customers WHERE company_id = ? AND phone = ? LIMIT 1`
        ).bind(companyId, phone).first();
        ...
        const existingRegisterSyncSucceeded = hasSuccessfulOdooRegisterSync(existing);
        ...
        const shouldAttemptRegisterSync = !skipRegisterSyncForTestException && (!existingRegisterSyncSucceeded || !alreadyActiveForRequestedProgram);

        if (shouldAttemptRegisterSync) {
          ...
          if (registerSyncResult.skipped) {
            ...
            await updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
              state: 'failed',
              error: registerSyncResult.error || 'Register sync failed',
              syncedAt: null,
              incrementAttempts: true
            });
            return Response.json({
              status: 'error',
              message: 'Profil konnte derzeit nicht mit Odoo synchronisiert werden. Bitte erneut versuchen.'
            }, { status: 502 });
          } else {
            await updateCustomerOdooRegisterSyncStatus(env, companyId, phone, {
              state: 'synced',
